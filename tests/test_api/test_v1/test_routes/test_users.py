...
def test_register_failed(client, mocker):
    data = {"username":"testuser","password":"testing"}
    mocker.patch("app.api.v1.services.user_service.create_user", return_value=False)
    response = client.post("/register",json=data)
...
def test_login(client, mocker):
    ...
    mocker.patch("app.api.v1.services.auth_service.verify_password", return_value=True)
...
def test_login_faield(client, mocker):
    data = {"username":"testuser","password":"testing"}
    mocker.patch("app.api.v1.services.user_service.get_user", return_value=None)
    mocker.patch("app.api.v1.services.auth_service.verify_password", return_value=False)
    response = client.post("/login",json=data)
